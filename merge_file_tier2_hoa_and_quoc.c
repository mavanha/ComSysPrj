...
int main() {
   ...
   // --- FIX: Wrap ALL debug printf in __...__ ---
   ...
   if (init_ICM42670() != 0) {
       printf("__CRITICAL ERROR: ICM42670 initialization failed!__\n");
       while (1) { blink_led(1); sleep_ms(100); }
   } else {
       printf("__ICM42670 initialized successfully.__\n");
       if (ICM42670_start_with_default_values() != 0) {
           printf("__ERROR: ICM42670 start failed!__\n");
   ...
   xTaskCreate(imu_task, "IMUTask", 2048, NULL, MAIN_TASK_PRIORITY, NULL);
       ...
       if (g_appState == STATE_ARMED || g_appState == STATE_COOLDOWN) {
           ...
           if (xSemaphoreTake(i2cMutex, pdMS_TO_TICKS(50)) == pdTRUE) {
               if (ICM42670_read_sensor_data(&ax, &ay, &az, &gx, &gy, &gz, &t) == 0) {
                   ...
                   // --- FIX: Wrap debug printf in __...__ ---
                   printf("__IMU Task: Failed to read sensor data.__\n");
               }
...
static void serial_tx_task(void *pvParameters) {
   ...
   while (1) {
       if (xQueueReceive(xSerialTxQueue, &symbol, portMAX_DELAY) == pdPASS) {
           ...
           // --- FIX: Simplified and Corrected TX Logic ---
           // This logic sends the symbol and *then* checks the space count
           ...
           if (symbol == ' ') {
               ...
               if (spaceCount == 3) {
                   ...
                   putchar('\n');
                   // --- FIX: Wrap debug printf in __...__ ---
                   // The \n *before* the __ is protocol, so it's OK.
                   printf("\n__[Morse Send OK]__\n");
                   fflush(stdout);
                   spaceCount = 0; // Reset
               }
...
static void serial_rx_task(void *pvParameters) {
   ...
   while (1) {
       ...
       if (c != PICO_ERROR_TIMEOUT) {
           // --- FIX: Removed serial echo (putchar(c)). ---
           // This was a protocol violation.
...
void process_received_line(char *line) {
   // --- FIX: Removed duplicate .clear and wrapped all feedback ---
   ...
   for (int i = 0; line[i] != '\0'; i++) {
       ...
       if (!in_debug_block) {
           ...
           if (symbol == '.' || symbol == '-' || symbol == ' ') {
               // --- FIX: Block on queue to prevent data loss ---
